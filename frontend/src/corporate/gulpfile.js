...
gulp.task('watch', function () {
	livereload.listen();
  gulp.watch('html/**/**.html', gulp.series(gulp.parallel(['fileinclude'])));
...
gulp.task('fileinclude', function() {
  return gulp.src(['./html/*.html'])
    ...
    .pipe(gulp.dest(distPath + '/template_'+ theme +''))
    .pipe(livereload());
});

gulp.task('webserver', function() {
  connect.server({
		name: 'Color Admin - Corporate',
		root: [distPath, distPath + '/template_'+ theme +'/'],
		port: 8000,
		livereload: true,
		fallback: 'index.html'
	});
